...
- **⚡️ 六大分散式能源資產 (DERs) 支援**
  系統完整內建了企業廠辦最常見的六類能源資產配置模型：包含 **Solar PV (自建光電)**、**PPA (綠電購電協議)**、**EV (電動車智慧充電)**、**ESS (儲能系統)**、**HVAC (空調負載移轉)** 以及 **DR (需量反應附屬服務)**。

- **🧠 Python 動態情境運算引擎**
  核心依賴 Python FastAPI 與 Pandas 建立高效模擬器。利用實際的 15 分鐘歷史負載數據，不僅能執行高精度的時間序列相加相減運算，還能進行為期「兩年的完整負載回測」。

- **🌍 賽博龐克風 2.5D 數位分身地圖**
  前端以 React、Vite、Tailwind CSS 所打造，具備充滿 Cyberpunk 未來科技感的 UI 介面。當使用者於面板中啟動特定的能源資產時，中央的數位分身園區地圖會「動態點亮」對應的 3D 設施。

- **📊 即時財務指標與減碳試算**
  所有資產配置牽一髮而動全身，系統會即刻為您計算涵蓋：**再生能源佔比 (RE%)**、**用電減少比例**、**累計算費節省 (NTD)**，甚至提供 **YoY (年增率)** 與 **MoM (月增率)** 的對比商業指標。

- **💡 AI 智慧洞察推薦 (Smart Insights)**
  根據您選擇的「資產組合策略」，引擎將自動為您生成深入的 AI 業務洞察。例如發現您啟用了光電與電動車，系統便會提出「Solar-to-EV 直充排程效益」來幫助決策，完美模擬一站式能源管理顧問服務。
